// functions/api/profile-photo.js — загрузить фото аватара профиля (то есть
// самого человека, а не персонажа). Тот же приём, что character-photo.js:
// фото уходит в Telegram Bot API самому загрузившему (sendPhoto), взамен
// получаем file_id - без него не нужен ни Supabase, ни R2.
...
export async function onRequestPost(context) {
  ...
  form.append("caption", "Аватар профиля.");
  ...
  if (!send.ok || !sendData.ok) {
    console.log("Ошибка отправки аватара профиля в Telegram:", JSON.stringify(sendData));
