// functions/api/story-photo.js — загрузить фото истории. Как и с аватаром
// персонажа/фото сообщения, фото отправляется через Telegram Bot API самому
// загрузившему (sendPhoto), взамен получаем file_id. Привязка к истории
// происходит отдельным запросом (публикация), потому что до публикации у
// истории ещё нет id.
...
export async function onRequestPost(context) {
  ...
  form.append("caption", "Фото для истории.");
  form.append("photo", new Blob([bytes], { type: contentType }), "story.jpg");
