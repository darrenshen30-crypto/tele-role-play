// functions/api/sms-photo.js — загрузить фото для SMS-сообщения. Как и с
// фото в чате локации, фото отправляется через Telegram Bot API самому
// загрузившему (sendPhoto), взамен получаем file_id. Привязка к сообщению
// происходит отдельным запросом (отправка), потому что до отправки у
// сообщения ещё нет id.
...
export async function onRequestPost(context) {
  ...
  form.append("caption", "Фото для SMS.");
  form.append("photo", new Blob([bytes], { type: contentType }), "sms.jpg");
